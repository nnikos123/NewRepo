...
namespace NLogServer.Protocol
{
    /// <summary>
    /// ReceiveFilter for the protocol that each request has bengin and end mark
    /// </summary>
    /// <typeparam name="TRequestInfo">The type of the request info.</typeparam>
    public abstract class NLogReceiveFilterA<TRequestInfo> : ReceiveFilterBase<TRequestInfo>
        where TRequestInfo : IRequestInfo
    {
        ...
        protected NLogReceiveFilterA(byte[] beginMark, byte[] endMark)
        {
        ...
        public override TRequestInfo Filter(byte[] readBuffer, int offset, int length, bool toBeCopied, out int rest)
        {
            ...
            string inp1 = BitConverter.ToString(readBuffer, offset, length);
            string inp2 = ASCIIEncoding.ASCII.GetString(readBuffer, offset, length);
            byte[] commandData2 = new byte[BufferSegments.Count + prevMatched + totalParsed];
            var requestInfo2 = ProcessMatchedRequest(commandData2, 0, commandData2.Length);
